...
class _SearchRoutineScreenState extends State<SearchRoutineScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Scaffold(
        appBar: AppBarWidget.header(context, 'Search routine'),
...
//    return ClassList.fromJson([{}]);
  ...
  Future<SectionList> getAllSection(int id, int classId) async {
    ...
    final response = await http.get(InfixApi.getSectionByClassId(id.toString(),classId.toString(),'getSectionByClassId',token));
    //print(response.body.toString());
    if (response.statusCode == 200) {
      var jsonData = jsonDecode(response.body);
